...
if(!empty($_GET['id'])){
    $UpdateId = $_GET['id'];
      ...
      A.*,
      A.`LastModifiedDate`,
      CONCAT(B.`UserName`,' - ', B.`Phone`,' - ', D.`ShopName`) AS `EntryInfo`,
      CONCAT(C.`UserName`,' - ', C.`Phone`,' - ', D.`ShopName`) AS `UpdateInfo`
      FROM `medicine` A 
      LEFT JOIN `user_information` B ON (A.`EntryID` = B.`Id`)
      LEFT JOIN `user_information` C ON (A.`UpdateId` = C.`Id`)
      LEFT JOIN `shop` D ON (A.`ShopId` = D.`Id`)  
      WHERE A.`MedicineID`='$UpdateId'");
      $sql->execute();
