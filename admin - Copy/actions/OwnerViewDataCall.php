...
$sql = "SELECT 
                ...
                DATE_FORMAT(B.`SubscriptionStartDate`, '%d/%m/%Y') AS SubscriptionStartDate,
                DATE_FORMAT(B.`SubscriptionEndDate`, '%d/%m/%Y') AS SubscriptionEndDate,
                B.`Status`,
                A.`CreateDate`
...
if(isset($_POST['search']['value']))
{
	...
	$sql .= "WHERE A.`UserName` like '%".$search_value."%'";
    $sql .= " OR A.`Phone` like '%".$search_value."%'";
    $sql .= " OR B.ShopName like '%".$search_value."%'";
    $sql .= " OR B.ShopAddress like '%".$search_value."%'";
    ...
    $sql .= " OR DATE_FORMAT(B.`SubscriptionStartDate`, '%d/%m/%Y') like '%".$search_value."%'";
    $sql .= " OR DATE_FORMAT(B.`SubscriptionEndDate`, '%d/%m/%Y') like '%".$search_value."%'";
    $sql .= " OR DATE_FORMAT(A.`CreateDate`, '%d/%m/%Y - %h:%i:%s %p') like '%".$search_value."%'";
    $sql .= " OR B.Status like '%".$search_value."%'";
...
foreach($result as $fetch)
{
    ...
    $sub_array[] = html_entity_decode($fetch["Phone"]);
