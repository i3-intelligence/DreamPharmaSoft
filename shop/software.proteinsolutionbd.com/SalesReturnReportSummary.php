...
$query = $conn->prepare("SELECT A.*,SUM(`ReturnAmount`) AS `TotalAmount`, CONCAT(B.`Name`,'-',B.`MobileNo`) AS `SupplierInfo`  FROM `CashMemoReturn` A 
LEFT JOIN `Supplier` B ON (A.`SupplierID` = B.`SupplierID`)
WHERE A.`Cart` = 'Yes' AND A.`CashMemoReturnDate` BETWEEN '$start_date' AND '$end_date'  $SupplierID $ItemCategoryID GROUP BY A.`CashMemoReturnInvoice` ORDER BY A.`CashMemoReturnInvoice` ASC");
$query->execute();
...
<?php
## Sales Return Report Invoice Details
...
<tr>
	...
	<th colspan="4" style="text-align:Center;"> Sales Return Invoice Details </th>
...
LEFT JOIN `ItemCategory` B ON (A.`ItemCategoryID` = B.`ItemCategoryID`)
LEFT JOIN `PackageSize` C ON (A.`PackageSizeID` = C.`PackageSizeID`)
WHERE A.`CashMemoReturnInvoice` = '$fetch[CashMemoReturnInvoice]'  ORDER BY B.`Name`,C.`Thickness` ASC");
$query2->execute();
