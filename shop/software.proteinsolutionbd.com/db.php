...
## FOR OFFLINE
	## FOR ONLINE
	$prefix = "proteinsolutionb";
	$servername = "localhost";
	$username = $prefix."_root";
	$Password = "@^!6AXWqG65XjeM^";
	$database = $prefix."_new";
...
if(!empty($_SESSION['PS_SESS_MEMBER_ID']))
{		//CAll User Information
		$query1 = $conn->prepare("SELECT A.*,B.`ShopName`,B.`ShopAddress`,B.`ShopContact`,B.`CreateDate` AS `ShopCreateDate`,B.`ShopDB` FROM `UserInformation` A
		 ...
		 WHERE A.`id` = '".$_SESSION['PS_SESS_MEMBER_ID']."' "); 
		$query1->execute();
