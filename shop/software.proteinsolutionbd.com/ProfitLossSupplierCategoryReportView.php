...
<html>
<head>
<title>Profit/Loss Supplier Category Wise Report >> <?php print $_GET['type']; ?></title>
...
// if(!empty($mpk[1]) && $mpk[1]=='1'){ 	
    ...
    if(!empty($_GET['SupplierCategoryID']) && $_GET['SupplierCategoryID'] !='All'){
        ...
        $query = $conn->prepare("SELECT * FROM `SupplierCategory` WHERE  `SupplierCategoryID` = '$SupplierCategoryID'"); 
        $query->execute();
...
<tr>
	<th style="text-align:center; font-size:18px;"> Profit/Loss Supplier Category Wise Report
	<?php if(!empty($_GET['type'])){ ?> (<?php print $_GET['type']; ?>) <?php } ?></th>
</tr>
<tr>
	<th style="font-size:14px; text-align :left"> Supplier Category Name : <?php print $Supplier; ?></th>
</tr>
</table>
...
<button id="download-button" class="dontPrint">Download CSV</button>
	...
	function downloadCSVFile(csv, filename) {
		...
		document.getElementById("download-button").addEventListener("click", function () {
			...
			htmlToCSV(html, "Profit/Loss Supplier Category Wise Report.csv");
		});
