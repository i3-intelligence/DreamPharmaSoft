...
if(isset($_POST['search']['value']))
{
	...
	$sql .= " WHERE 
	`SupplierCategoryID` like '%".$search_value."%'";
...
{
    $sql .= " ORDER BY `Name`,LPAD(`SupplierCategoryID`, 3, '0') ASC";
}
...
foreach($result as $fetch)
{
    ...
    $sub_array[] = html_entity_decode(sprintf("%03d",$fetch["SupplierCategoryID"]));
    $sub_array[] = html_entity_decode($fetch["SupplierCategoryName"]);
	$sub_array[] = "<span class=\"$status_span\">$fetch[Status]</span>";
						...
						data-whatever=\"$fetch[SupplierCategoryID]\">
