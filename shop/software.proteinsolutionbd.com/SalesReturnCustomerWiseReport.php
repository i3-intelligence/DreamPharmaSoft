...
<html>
<head>
<title>Sales Return Report >> <?php print $_GET['type']; ?></title>
...
// if(!empty($mpk[1]) && $mpk[1]=='1'){ 	
    ...
    if(!empty($_GET['SupplierID2']) && $_GET['SupplierID2'] !='All'){
        $SupplierID = $_GET['SupplierID2'];
...
<tr>
	<th style="text-align:center; font-size:18px;"> Sales Return Summary Report 
...
<?php
## Sales Return Report Invoice Wise
...
<tr>
	...
	<th> Date </th>
	<th> Customer </th>
	<th> Sales Return Invoice </th>
	<th> Sales Return Invoice Amount </th>
	<!-- <th> Discount </th>
	<th> Transport </th>
	<th> Payable </th> -->
</tr>
...
if(!empty($_GET['ItemCategoryID2']) && $_GET['ItemCategoryID2'] =='All'){
    $ItemCategoryID ="";
}else{
    $ItemCategoryID =" AND A.`ItemCategoryID` = '$_GET[ItemCategoryID2]' ";
} 
...
if(!empty($_GET['SupplierID2']) && $_GET['SupplierID2'] =='All'){
    ...
    $SupplierID =" AND A.`SupplierID` = '$_GET[SupplierID2]' ";
} 
...
$query = $conn->prepare("SELECT A.*,SUM(`ReturnAmount`) AS `TotalAmount`, CONCAT(B.`Name`,'-',B.`MobileNo`) AS `CustomerInfo`  FROM `CashMemoReturn` A 
LEFT JOIN `Customer` B ON (A.`CustomerID` = B.`CustomerID`)
WHERE A.`Cart` = 'Yes' AND A.`CashMemoReturnDate` BETWEEN '$start_date' AND '$end_date'  $CustomerID $ItemCategoryID $SupplierID AND A.`CustomerID` !='0'  GROUP BY A.`CashMemoReturnInvoice` ORDER BY A.`CashMemoReturnInvoice` ASC");
$query->execute();
...
<tr>
	<td><?php echo $sl++; ?> </td>
    <td style="text-align:left;"> <?php print date("d-m-Y",strtotime($fetch['CashMemoReturnDate'])); ?> </td>
...
'mywindow','menubar=1,resizable=1,width=900,height=800');" ><?php echo $fetch['CashMemoReturnInvoice']; ?></a>
...
B.`Name`,
SUM(A.`ReturnQuantity`) AS `Quantity`, 
SUM(A.`ReturnAmount`) AS `TotalAmount`,
...
<?php
## Sales Return ReportInvoice Details
...
<tr>
	...
	<th rowspan="2"> Sales Return Invoice </th>
	<th rowspan="2">Customer Name </th>
	<th colspan="5" style="text-align:Center;"> Sales Return Invoice Details </th>
</tr>
<tr>
    <th>Product</th>
    <th>Qty</th>
...
$NetKg = 0;
$query = $conn->prepare("SELECT 
                            COUNT(A.`CashMemoReturnID`) AS `total_data`,
                            A.`CashMemoReturnInvoice`,
                            A.`CashMemoReturnDate`,
							A.`Remarks`,
                            SUM(A.`ReturnAmount`) AS `TotalAmount` ,
                            SUM(A.`ReturnQuantity`) AS `TotalQuantity` ,

                            (CASE WHEN  A.`CustomerID` = '0'  THEN A.`CustomerName` ELSE CONCAT(B.`CustomerID`,'-',B.`Name`,'-',B.`MobileNo`) END) AS `CustomerInfo`

                        FROM `CashMemoReturn` A 
                        LEFT JOIN `Customer` B ON A.`CustomerID` = B.`CustomerID` 
WHERE A.`Cart` = 'Yes' AND  A.`CashMemoReturnDate` BETWEEN '$start_date' AND '$end_date'  $CustomerID $ItemCategoryID $SupplierID GROUP BY A.`CashMemoReturnInvoice` ORDER BY A.`CashMemoReturnInvoice` ASC");
$query->execute();
...
<tr>
    ...
    <td style="text-align:left;" rowspan="<?php print $fetch['total_data']; ?>"> <?php print date("d-m-Y",strtotime($fetch['CashMemoReturnDate'])); ?> </td>
...
'mywindow','menubar=1,resizable=1,width=1000,height=800');" ><?php print $fetch['CashMemoReturnInvoice']; ?></a>
	...
	<td  rowspan="<?php print $fetch['total_data']; ?>" style="color:blue;" ><?php print wordwrap($fetch['CustomerInfo'],20, "<br>"); ?></td>
...
LEFT JOIN `ItemCategory` B ON (A.`ItemCategoryID` = B.`ItemCategoryID`)
LEFT JOIN `PackageSize` C ON (A.`PackageSizeID` = C.`PackageSizeID`)
WHERE A.`CashMemoReturnInvoice` = '$fetch[CashMemoReturnInvoice]' $ItemCategoryID $SupplierID   ORDER BY B.`Name`,C.`Thickness` ASC");
$query2->execute();
...
?>
    ...
    <td style="text-align:left;"> <?php print $fetch2['ItemCategory']; ?> &times;  <?php print $fetch2['Thickness']; ?>  <?php print $fetch2['Size']; ?></td>
    <td style="text-align:right;"> <?php print $fetch2['ReturnQuantity']; ?> </td>
    <td style="text-align:right;"> <?php print $KG =($fetch2['ReturnQuantity'] * $fetch2['Thickness']); ?>  <?php print $fetch2['Size']; ?></td>
...
$CashMemoReturnAmount += $fetch['TotalAmount'];
$NetKg += $TotalKg;
...
<tfoot>
<tr>
	<th style="text-align:center;" colspan="5">NET TOTAL </th>
	<th style="text-align:right;"><?php print number_format($TotalQuantity,2,'.',''); ?></th>
	<th style="text-align:right;"><?php print number_format($NetKg,2,'.',''); ?></th>
	<th></th>
