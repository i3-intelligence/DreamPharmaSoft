...
<html>
<head>
<title>Profit/Loss Customer Category Wise Report >> <?php print $_GET['type']; ?></title>
...
if(!empty($_GET['CustomerSubCategoryID']) && $_GET['CustomerSubCategoryID'] !='All'){
	...
	$query = $conn->prepare("SELECT * FROM `CustomerSubCategory` WHERE  `CustomerSubCategoryID` = '$CustomerSubCategoryID'"); 
	$query->execute();
...
<tr>
	<th style="text-align:center; font-size:18px;"> Profit/Loss Customer Sub Category Wise Report
	<?php if(!empty($_GET['type'])){ ?> (<?php print $_GET['type']; ?>) <?php } ?></th>
</tr>
<tr>
	<th style="font-size:14px; text-align :left"> Customer Sub Category Name : <?php print $Customer; ?></th>
</tr>
</table>
...
if(!empty($_GET['CustomerSubCategoryID']) && $_GET['CustomerSubCategoryID'] =='All'){
    ...
    $CustomerSubCategoryID =" AND E.`CustomerSubCategoryID` = '$_GET[CustomerSubCategoryID]' ";
} 
...
AND A.`CashMemoDate` 
BETWEEN '$start_date' 
AND '$end_date' AND A.`CustomerID` != '0' $CustomerSubCategoryID 
...
'mywindow','menubar=1,resizable=1,width=1000,height=800');" ><?php print $fetch['CashMemoReturnInvoice']; ?></a>
    ...
    <th style="text-align:right; <?php if($Profit >='1'){ print "color:green;"; }?>"> <?php print number_format($Profit,2,'.',''); ?> </th>
    <th style="text-align:right; <?php if($Profit <= '1'){ print "color:red;"; }?>"> <?php print number_format($Loss,2,'.',''); ?> </th>
</tr>
...
<button id="download-button" class="dontPrint">Download CSV</button>
	...
	function downloadCSVFile(csv, filename) {
		...
		document.getElementById("download-button").addEventListener("click", function () {
			...
			htmlToCSV(html, "Profit/Loss Customer Category Wise Report.csv");
		});
