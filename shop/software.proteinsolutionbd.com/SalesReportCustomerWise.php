...
$query = $conn->prepare("SELECT A.*,SUM(`SalesAmount`) AS `TotalAmount`, CONCAT(B.`Name`,'-',B.`MobileNo`) AS `CustomerInfo`  FROM `CashMemo` A 
LEFT JOIN `Customer` B ON (A.`CustomerID` = B.`CustomerID`)
WHERE A.`Cart` = 'Yes' AND A.`CashMemoDate` BETWEEN '$start_date' AND '$end_date'  $CustomerID $ItemCategoryID $SupplierID AND A.`CustomerID` !='0'  GROUP BY A.`CashMemoInvoice` ORDER BY A.`CashMemoInvoice` ASC");
$query->execute();
...
$query = $conn->prepare("SELECT 
                        ...
                        FROM `CashMemo` A 
                        LEFT JOIN `Customer` B ON A.`CustomerID` = B.`CustomerID` 
WHERE A.`Cart` = 'Yes' AND  A.`CashMemoDate` BETWEEN '$start_date' AND '$end_date'  $CustomerID $ItemCategoryID $SupplierID GROUP BY A.`CashMemoInvoice` ORDER BY A.`CashMemoInvoice` ASC");
$query->execute();
...
LEFT JOIN `ItemCategory` B ON (A.`ItemCategoryID` = B.`ItemCategoryID`)
LEFT JOIN `PackageSize` C ON (A.`PackageSizeID` = C.`PackageSizeID`)
WHERE A.`CashMemoInvoice` = '$fetch[CashMemoInvoice]' $ItemCategoryID $SupplierID   ORDER BY B.`Name`,C.`Thickness` ASC");
$query2->execute();
