...
<?php
$query = $conn->prepare("SELECT *,date_format(`ChallanDate`,'%d-%m-%Y') AS `mdate` FROM `Challan` WHERE `ChallanInvoice` = '$_GET[ChallanInvoice]'   AND `Cart` = 'Yes' GROUP BY `ChallanInvoice`"); 
$query->execute();
if($query->rowCount()==0)	{

	echo "<h1 style='text-align:center; color:red;'>Challan Invoice Not Found !!!</h1>";
	exit();
	}
	...
	<div id="page-wrap">
		...
		<div id="identity" style="border:0px solid; width:100%; float:left;">
           ...
           <table id="meta" style="width: 250px;">
                ...
                <tr>
                    <td style="border: none;text-align: left;font-size:15px;padding: 0px;"><b>Challan No.:</b> </td>
                    <td style="border: none;text-align: left;font-size:15px;padding: 0px;"><b><?php print $_GET['ChallanInvoice']; ?></b></td>
                </tr>
...
<tr>
  ...
  <th style="width: 15%">Challan Qty</th>
...
WHERE A.`ChallanInvoice` = '".$_GET['ChallanInvoice']."' AND A.`Cart` = 'Yes' 
...
<tr class="item-row">
  ...
  <td style="text-align:right;border-left: solid 1px;"><b><?php print $Fetch['Rate']; ?></b></td>
  <td style="text-align:right;border-left: solid 1px;"><b><?php print $Fetch['Quantity']; ?></b></td>
  <td style="text-align:right;border-left: solid 1px;"><b><?php print $Fetch['Amount']; ?></b></td>
</tr>

<?php  
$qty +=$Fetch['Quantity'];
$gdelivery_rate +=$Fetch['Rate'];
$gdelivery_total_amount +=$Fetch['Amount'];
