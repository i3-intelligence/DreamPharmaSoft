...
if(isset($_POST['search']['value']))
{
	...
	$sql .= " WHERE 
	`CustomerCategoryID` like '%".$search_value."%'";
...
{
    $sql .= " ORDER BY `Name`,LPAD(`CustomerCategoryID`, 3, '0') ASC";
}
...
foreach($result as $fetch)
{
    ...
    $sub_array[] = html_entity_decode(sprintf("%03d",$fetch["CustomerCategoryID"]));
    $sub_array[] = html_entity_decode($fetch["CustomerCategoryName"]);
	$sub_array[] = "<span class=\"$status_span\">$fetch[Status]</span>";
						...
						data-whatever=\"$fetch[CustomerCategoryID]\">
