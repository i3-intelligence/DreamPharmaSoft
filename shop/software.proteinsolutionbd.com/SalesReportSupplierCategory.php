...
// if(!empty($mpk[1]) && $mpk[1]=='1'){ 	
    ...
    if(!empty($_GET['SupplierCategoryID']) && $_GET['SupplierCategoryID'] !='All'){
        $SupplierCategoryID = $_GET['SupplierCategoryID'];
        $query = $conn->prepare("SELECT `Name` FROM `SupplierCategory` WHERE  `SupplierCategoryID` = '$SupplierCategoryID'"); 
        $query->execute();
        $SupplierCategoryID = $query->fetch();
        $SupplierCategoryID = $SupplierCategoryID['Name'];
        }else{
...
<tr>
	<th style="text-align:center; font-size:18px;">  Supplier Category Wise Sales Report 
	<?php if(!empty($_GET['type'])){ ?> (<?php print $_GET['type']; ?>) <?php } ?></th>
</tr>
<tr>
	<th style="font-size:14px; text-align :left">Supplier Category Name : <?php print $SupplierCategoryID; ?></th>
</tr>
</table>
...
if(!empty($_GET['SupplierCategoryID']) && $_GET['SupplierCategoryID'] =='All'){
    ...
    $SupplierCategoryID =" AND B.`SupplierCategoryID` = '$_GET[SupplierCategoryID]' ";
} 
...
WHERE A.`Cart` = 'Yes' AND A.`CashMemoDate` BETWEEN '$start_date' AND '$end_date' $SupplierCategoryID GROUP BY A.`CashMemoInvoice` ORDER BY A.`CashMemoInvoice` ASC");
$query->execute();
...
AND A.`CashMemoDate` 
BETWEEN '$start_date' 
AND '$end_date' $SupplierCategoryID  
...
if(!empty($_GET['SupplierCategoryID']) && $_GET['SupplierCategoryID'] =='All'){
    ...
    $SupplierCategoryID =" AND D.`SupplierCategoryID` = '$_GET[SupplierCategoryID]' ";
} 
...
LEFT JOIN `Supplier` D ON (A.`SupplierID` = D.`SupplierID`)
LEFT JOIN `PackageSize` C ON (A.`PackageSizeID` = C.`PackageSizeID`)
WHERE A.`CashMemoInvoice` = '$fetch[CashMemoInvoice]' $SupplierCategoryID  ORDER BY D.`Name`,C.`Thickness` ASC");
$query2->execute();
...
?>
    ...
    <td style="text-align:left;"> <?php print $fetch2['Supplier']; ?> &times;  <?php print $fetch2['Thickness']; ?> </td>
...
<button id="download-button" class="dontPrint">Download CSV</button>
	...
	function downloadCSVFile(csv, filename) {
		...
		document.getElementById("download-button").addEventListener("click", function () {
			...
			htmlToCSV(html, "Supplier Category Wise Sales Report.csv");
		});
