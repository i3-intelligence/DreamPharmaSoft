...
$TableName = "temp_CustomerBalance".$SessionID;
...
SELECT 
        ...
        `EntryID` 
    FROM `CashMemo` WHERE `CustomerID` = '".$CustomerID."'  AND `Cart` = 'Yes' AND `CashMemoDate` BETWEEN '".$i3_define_date."' AND '".$CurrentDate."' GROUP BY `CashMemoInvoice`");
...
SELECT 
        ...
        `TransactionType` AS `PaymentType`, 
        `EntryID` 
    FROM `CashMemoReturn` WHERE `CustomerID` = '".$CustomerID."'  AND `Cart` = 'Yes' AND `CashMemoReturnDate` BETWEEN '".$i3_define_date."' AND '".$CurrentDate."' GROUP BY `CashMemoReturnInvoice`");
...
SELECT 
    ...
    `EntryID` 
FROM `Receive` WHERE `CustomerID` = '".$CustomerID."'  AND `ReceiveDate` BETWEEN '".$i3_define_date."' AND '".$CurrentDate."' AND CustomerDueReceiveInvoice != '' GROUP BY `CustomerDueReceiveInvoice` ");
...
SELECT 
    ...
    `PaymentName` AS `PaymentType`, 
    `EntryID` 
FROM `Receive` WHERE `CustomerID` = '".$CustomerID."'  AND `ReceiveDate` BETWEEN '".$i3_define_date."' AND '".$CurrentDate."' AND CustomerReceiveInvoice != '' GROUP BY `CustomerReceiveInvoice` ");
...
(IFNULL(B.`Payable`,0) - (IFNULL(B.`Receivable`,0) + IFNULL(B.`Discount`,0))) AS `Balance`
 ...
 LEFT JOIN (SELECT
  ...
  FROM `$TableName` WHERE  `Date` BETWEEN '".$i3_define_date."' AND '".$CurrentDate."' GROUP BY `CustomerID`  ) B ON (A.`CustomerID` = B.`CustomerID`)
 WHERE A.`CustomerID` = '".$CustomerID."' ");
$Balance_qurry->execute();
$OpeningBalance = $Balance_qurry->fetch(PDO::FETCH_ASSOC);

$CustomerBalance = number_format($OpeningBalance['OpeningBalance'] + $OpeningBalance['Balance'],2,'.','');
