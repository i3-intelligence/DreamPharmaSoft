...
$sql = "SELECT 	
				A.*,
				B.`Name` AS `CustomerCategory`,
                C.`Name` AS `CustomerSubCategory`
		FROM `Customer` A 
		LEFT JOIN `CustomerCategory` B ON (A.`CustomerCategoryID` = B.`CustomerCategoryID`) 
        LEFT JOIN `CustomerSubCategory` C ON (A.`CustomerSubCategoryID` = C.`CustomerSubCategoryID`)
        ";
...
if(isset($_POST['search']['value']))
{
	...
	$sql .= " WHERE 
	A.`CustomerID` like '%".$search_value."%'";
	...
	$sql .= " OR A.`CreditLimit` like '%".$search_value."%'";
...
{
    $sql .= " ORDER BY A.`Name`,A.`MobileNo`,LPAD(A.`CustomerID`, 3, '0') ASC";
}
...
foreach($result as $fetch)
{
    ...
    $sub_array[] = html_entity_decode(sprintf("%03d",$fetch["CustomerID"]));
    $sub_array[] = html_entity_decode($fetch["Name"]);
    $sub_array[] = html_entity_decode($fetch["CustomerCategory"]);
    $sub_array[] = html_entity_decode($fetch["CustomerSubCategory"]);
	...
	$sub_array[] = "<span class=\"$status_span\">$fetch[Status]</span>";
						...
						data-whatever=\"$fetch[CustomerID]\">
