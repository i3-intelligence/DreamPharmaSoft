...
$output= array();
$sql = "SELECT A.`ItemCategoryID`,A.`Name` AS `ItemCategoryName`,CONCAT(B.`Name`,' - ', B.`MobileNo`,' - ', B.`Address`) AS `SupInfo`,A.`Status` FROM `ItemCategory` A LEFT JOIN `Supplier` B ON (A.`SupplierID` = B.`SupplierID`) ";
...
if(isset($_POST['search']['value']))
{
	...
	$sql .= " WHERE 
	A.`ItemCategoryID` like '%".$search_value."%'";
...
{
    $sql .= " ORDER BY A.`Name`,LPAD(A.`ItemCategoryID`, 3, '0') ASC";
}
...
foreach($result as $fetch)
{
    ...
    $sub_array[] = html_entity_decode(sprintf("%03d",$fetch["ItemCategoryID"]));
    $sub_array[] = html_entity_decode($fetch["ItemCategoryName"]);
	...
	$sub_array[] = "<span class=\"$status_span\">$fetch[Status]</span>";
						...
						data-whatever=\"$fetch[ItemCategoryID]\">
