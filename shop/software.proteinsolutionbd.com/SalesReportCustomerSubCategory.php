...
// if(!empty($mpk[1]) && $mpk[1]=='1'){ 	
    ...
    if(!empty($_GET['CustomerSubCategoryID']) && $_GET['CustomerSubCategoryID'] !='All'){

    $CustomerSubCategoryID = $_GET['CustomerSubCategoryID'];
    $query = $conn->prepare("SELECT CONCAT(`CustomerSubCategoryID`,'-',`Name`) AS `CustomerSubCategoryInfo` FROM `CustomerSubCategory` WHERE  `CustomerSubCategoryID` = '$CustomerSubCategoryID' "); 
    $query->execute();
    $CustomerSubCategory = $query->fetch();
      $CustomerSubCategoryInfo = $CustomerSubCategory['CustomerSubCategoryInfo'];
    }else{
...
<tr>
	<th style="text-align:center; font-size:18px;"> Sales Report Customer Sub Category Wise Report
...
if(!empty($_GET['CustomerSubCategoryID']) && $_GET['CustomerSubCategoryID'] =='All'){
    ...
    $CustomerSubCategoryID =" AND B.`CustomerSubCategoryID` = '$_GET[CustomerSubCategoryID]' ";
}   
...
$query = $conn->prepare("SELECT A.*,SUM(`SalesAmount`) AS `TotalAmount`, CONCAT(B.`Name`,'-',B.`MobileNo`,'-<b>',C.`Name`,'</b>') AS `CustomerInfo`  FROM `CashMemo` A 
LEFT JOIN `Customer` B ON (A.`CustomerID` = B.`CustomerID`)
LEFT JOIN `CustomerSubCategory` C ON (B.`CustomerSubCategoryID` = C.`CustomerSubCategoryID`)
WHERE A.`Cart` = 'Yes' AND A.`CashMemoDate` BETWEEN '$start_date' AND '$end_date'  $CustomerSubCategoryID  AND A.`CustomerID` !='0'  GROUP BY A.`CashMemoInvoice` ORDER BY A.`CashMemoInvoice` ASC");
$query->execute();
...
if(!empty($_GET['CustomerSubCategoryID']) && $_GET['CustomerSubCategoryID'] =='All'){
    $CustomerSubCategoryID ="AND D.`CustomerSubCategoryID` !=  '0' ";
}else{
    $CustomerSubCategoryID =" AND D.`CustomerSubCategoryID` = '$_GET[CustomerSubCategoryID]' ";
}   
...
AND A.`CashMemoDate` 
BETWEEN '$start_date' 
AND '$end_date' $CustomerSubCategoryID 
...
<tr>
	...
	<th colspan="5" style="text-align:Center;"> Sales Invoice Details </th>
</tr>
<tr>
    <th>Product</th>
    <th>Qty</th>
...
$query = $conn->prepare("SELECT 
                            ...
                            (CASE WHEN  A.`CustomerID` = '0'  THEN A.`CustomerName` ELSE CONCAT(D.`CustomerID`,'-',D.`Name`,'-',D.`MobileNo`) END) AS `CustomerInfo`

                        FROM `CashMemo` A 
                        LEFT JOIN `Customer` D ON A.`CustomerID` = D.`CustomerID` 
WHERE A.`Cart` = 'Yes' AND  A.`CashMemoDate` BETWEEN '$start_date' AND '$end_date'  $CustomerSubCategoryID  GROUP BY A.`CashMemoInvoice` ORDER BY A.`CashMemoInvoice` ASC");
$query->execute();
...
LEFT JOIN `PackageSize` C ON (A.`PackageSizeID` = C.`PackageSizeID`)
LEFT JOIN `Customer` D ON (A.`CustomerID` = D.`CustomerID`)
WHERE A.`CashMemoInvoice` = '$fetch[CashMemoInvoice]' $CustomerSubCategoryID  ORDER BY B.`Name`,C.`Thickness` ASC");
$query2->execute();
...
?>
    ...
    <td style="text-align:right;"> <?php print $fetch2['SalesQuantity']; ?> </td>
    <td style="text-align:right;"> <?php print $KG =($fetch2['SalesQuantity'] * $fetch2['Thickness']); ?>  <?php print $fetch2['Size']; ?></td>
